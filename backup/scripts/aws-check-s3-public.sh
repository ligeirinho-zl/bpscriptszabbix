...
function check_s3_homologation(){
  ...
  aws --profile $PROFILE_AWS --region sa-east-1 s3 ls | cut -d " " -f3 > $S3_OUTPUT_LIST
  while IFS= read -r line
    do
      aws s3 --profile $PROFILE_FAKE --region sa-east-1 cp $POC_FILE s3://$line/poc.txt 1>> $RESULT 2>> /dev/null
      COUNT=$((COUNT + 1))
