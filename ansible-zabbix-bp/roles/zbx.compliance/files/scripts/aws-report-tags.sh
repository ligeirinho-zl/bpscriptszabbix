...
get_resource_compliance_line 1 EBT "$(aws elasticbeanstalk describe-applications --query "Applications[].ApplicationArn" --output json | jq -r '.[]' )" 'elasticbeanstalk'
...
get_resource_compliance_line 3 CFT "$(aws cloudfront list-distributions --query "DistributionList.Items[].ARN" --output json | jq -r '.[]')" 'cloudfront'
get_resource_compliance_line 4 ECS "$(aws ecs list-clusters --query "clusterArns[*]" --output json | jq -r '.[]')" 'ecs'
get_resource_compliance_line 5 IAM "$(aws iam list-roles --query 'Roles[*].RoleName' --output json | jq -r '.[]')" 'iam'
get_resource_compliance_line 6 LBD "$(aws lambda list-functions --query 'Functions[*].FunctionArn' --output json | jq -r '.[]')" 'lambda'
